...
(define-language STAN
  ...
  ;; types
  (t ::= i r v a m) ; int real vector array matrix
...
(define-metafunction STAN
  extend : ((x EV) ...) x C t EV -> ((x EV C t) ...)
  [(extend () x EV C t) ((x EV C t))]
  
  [(extend ((x EV) (x_rest EV_rest) ...) x_1 EV_1 C_any t_any)
    ((x EV_1 C t) (x_rest EV_rest C_rest t_rest) ...)
...
;; TODO: vec cannot be set to a a real or int
;; TODO: vec cannot be set to a differentt type of vec (ordered, etc.)
;; TODO: vec cannot be set to an array
;; TODO: vec cannot be set to a matrix
;; TODO: real or int cannot be set to a vec/array/matrix
...
(define vc4 (apply-reduction-relation* stan_r (term ((v none x) ()))))
